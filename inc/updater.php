...
class WPCom_Theme_Updater {
	...
	public static function init() {
		...
		// Only for multisite users not running this as a plugin.
		if ( is_multisite() && ! ( str_starts_with( __FILE__, WP_PLUGIN_DIR ) || str_starts_with( __FILE__, WPMU_PLUGIN_DIR ) ) ) {
			add_action( 'admin_notices', [ __CLASS__, 'maybe_nag_for_plugin' ] );
			add_action( 'wp_ajax_wpcom-theme-updates-hide-nag', [ __CLASS__, 'hide_plugin_nag' ] );
		}
	}
	...
	public static function check_for_updates( $update_data ) {
		...
		$wpcom_themes_to_check = array_filter( array_keys( $update_data->checked ), [ __CLASS__, 'is_wpcom_slug' ] );
		...
		if ( empty( $response_data ) || ! is_array( $response_data ) || ! isset( $response_data['updates'] ) ) {
			return $update_data;
		}
	...
	/**
	 * Emails the network site admin about installing the WP.com Theme Updater plugin.
	 * Will only email once.
	 *
	 * @return void
	 */
	...
	protected static function output_notice() {
		...
		$install_url = wp_nonce_url(
			network_admin_url( 'update.php?action=install-plugin&plugin=' . self::PLUGIN_SLUG ),
		...
		<script>
		(function() {
			var hideLink = document.getElementById( 'wpcom-hide-nag' );
			if ( ! hideLink ) {
				return;
			}
			...
			hideLink.addEventListener( 'click', function( e ) {
				...
				params.append( 'action', 'wpcom-theme-updates-hide-nag' );
				params.append( 'nonce',  '<?php echo wp_create_nonce( 'wpcom-theme-updates-hide-nag' ); ?>' );
				...
				var nag = document.getElementById( 'wpcom-nag' );
				if ( nag ) {
					nag.classList.add( 'wpcom-nag--hidden' );
				}
			} );
		})();
		</script>
	 ...
	 * @param mixed  $value The value to set in $key.
	 * @return void
	 */
	...
	public static function is_wpcom_slug( $slug ) {
		return (bool) preg_match( '/-wpcom$/', $slug );
	}
